...
SERALYNX LLC / Porta SFTP Server is also developing its own SFTP/SCP Server and a next-generation secure transfer protocol — **HACLEX™ (High-Acceleration & Cryptographic Layer Exchange, TCP + UDP)** — designed to address limitations in the classic SSH/SFTP model for modern critical infrastructure workflows. HACLEX™ is being built for SERALYNX's future and existing proprietary products and is currently in internal research and testing. It will be revealed after an extended validation period (months to years). No ETA is available at this time.
